...
export const TOPICS_PLUGIN_MANIFEST = Object.freeze({ id: 'omos.topics', name: 'Topics', version: '2.0.0', kind: 'bundled-first-party' })
export const TOPICS_VERSION = 2
const MAX_ATTEMPTS = 5; const LEASE_MS = 120_000; const MAX_BACKOFF_MS = 30 * 60 * 1000; const MAX_TOPICS = 5; const MAX_TOPIC_LENGTH = 50
...
export function getArticleTopics(articleId: number): ArticleTopics | null {
  const row = getDb().prepare('SELECT * FROM article_topics WHERE article_id = ?').get(articleId) as { topics_json: string; source_content_hash: string; topics_version: number; created_at: string; updated_at: string } | undefined
...
export function validateTopics(value: unknown): string[] { if (!Array.isArray(value) || value.length < 1 || value.length > MAX_TOPICS) throw new Error('Topics must be a list of 1 to 5 subjects'); const topics = value.map(v => typeof v === 'string' ? v.trim() : ''); const filler = /^(news|article|update|latest news|general|other|miscellaneous|various topics)$/i; const technical = /(api|sdk|https?|json|javascript|typescript|python|css|sql|endpoint|implementation|code|software|database|algorithm|framework|library|protocol|repository|deployment|backend|frontend|pipeline|debugging|tutorial)/i; if (topics.some(v => !v || v.length > MAX_TOPIC_LENGTH || v.split(/\s+/).length > 4 || /https?:\/\//i.test(v) || /[{}()[\]_=<>]|=>|\\|::/.test(v) || filler.test(v) || technical.test(v) || /[.!?]/.test(v))) throw new Error('Topics must be concise broad noun phrases'); if (new Set(topics.map(v => v.toLowerCase())).size !== topics.length) throw new Error('Topics must be distinct'); return topics }
...
export async function runTopicsJobs(options: { batchSize?: number; concurrency?: number; now?: number; random?: () => number } = {}): Promise<number> {
  ...
  let next = 0; const worker = async () => { while (next < jobs.length) { const job = jobs[next++]; if (!job.lease_token) continue; try {
    ...
    db.transaction(() => { db.prepare(`INSERT INTO article_topics (article_id, topics_json, source_content_hash, topics_version) VALUES (?, ?, ?, ?) ON CONFLICT(article_id) DO UPDATE SET topics_json = excluded.topics_json, source_content_hash = excluded.source_content_hash, topics_version = excluded.topics_version, updated_at = datetime('now')`).run(job.article_id, JSON.stringify(topics), job.content_hash, TOPICS_VERSION); db.prepare("UPDATE topics_jobs SET status = 'succeeded', topics_version = ?, lease_token = NULL, lease_expires_at = NULL, completed_at = datetime('now'), updated_at = datetime('now') WHERE id = ? AND lease_token = ?").run(TOPICS_VERSION, job.id, job.lease_token) })()
