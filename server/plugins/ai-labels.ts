...
export interface AiLabelCandidate { name: string; confidence: number }
...
export function validateAiLabels(value: unknown): AiLabelCandidate[] {
  if (!Array.isArray(value) || value.length > 3) throw new Error('AI labels must contain at most 3 candidates')
...
function findOrCreateLabel(name: string): number {
  ...
  const info = db.prepare("INSERT INTO labels (name, match_text, match_field, sort_order, auto_summarize, exclusive, origin, normalized_name) VALUES (?, '', 'both', (SELECT COALESCE(MAX(sort_order), -1) + 1 FROM labels), 0, 0, 'ai', ?)").run(name, normalized)
...
export async function runAiLabelJobs(options: { batchSize?: number; now?: number } = {}): Promise<number> {
  ...
  for (const job of jobs) {
    ...
    try {
      ...
      const candidates = validateAiLabels(await extractAiLabels(article.full_text))
      ...
      db.transaction(() => {
        ...
        for (const candidate of candidates.filter(item => item.confidence >= MIN_CONFIDENCE)) { const labelId = findOrCreateLabel(candidate.name); if (labelId) insert.run(job.article_id, labelId, candidate.confidence, job.content_hash) }
